...
class TreeModal extends Component {
  ...
  close() {
    this.props.treeEntryStore.setTreeModal(false)
  }
  ...
  submit(e) {
    ...
    this.props.form.validateFields(async(err, values) => {
        if (err) {
            ...
            console.log(<FormattedMessage id="js.com.tre4.0002" defaultMessage="提交成功" />, values)
             // rootMainfolder
            const { nodeType, nodeInfo } = this.props.treeEntryStore.modal
            // debugger
            console.log('nodeType', nodeType)
            if (nodeType === 'branchAdd' ) {
              // 如果是主数据 接口名称：/modeling/mdmdesign/save
              let leafNode = await this.props.treeEntryStore.submitLeafRequest(values)
              this.close()
              const { expandedKeys } = this.props.treeEntryStore.tree;
              await this.props.treeEntryStore.getTree(this.props.url, this.props.expendId)
              if (leafNode && leafNode.id) {
                if (!expandedKeys.includes(leafNode.pid)) {
                  expandedKeys.push(leafNode.pid)
                }
                // 复制成功保存上级节点名称
                this.props.treeEntryStore.setParentName(nodeInfo.name)
              ...
              let leafNode = await this.props.treeEntryStore.submitAddNodeRequest(values)
              this.close()
              await this.props.treeEntryStore.getTree(this.props.url, this.props.expendId)
              // if (leafNode && leafNode.id) {
              ...
              // 修改编辑
              const { nodeInfo,editInfo } = this.props.treeEntryStore.modal;
              values.pk_category = editInfo.treeref_pkgd;
              console.log(values)
              switch(nodeInfo.isparent){
                case true: //树枝干
                  await this.props.treeEntryStore.submitEditNodeRequest(values);
                  break;
                case false: //叶子节点
                  await this.props.treeEntryStore.getDesignInfo(nodeInfo.id);
                  await this.props.treeEntryStore.submitLeafRequest(values,nodeInfo.id);
                  break;
              }
              this.close()
	...
	render() {
    const { modal, ifDisplayNoneClass,nodeLeaf:{info:{state}} } = this.props.treeEntryStore
    ...
    // console.log('ifDisplayNoneClass', ifDisplayNoneClass)
      ...
      <div>
                ...
                >
                  ...
                  <Button shape="border" disabled={flag && (state === 1 || state === 2)} style={{ marginRight: 20 }} onClick={this.close}>
