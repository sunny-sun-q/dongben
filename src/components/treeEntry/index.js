...
import { Icon, Menu, Button, Tooltip, Upload  } from 'tinper-bee';
...
class Siderbartree extends Component {
  ...
  onTreeSelect = (selectedKeys, info) => {
    // this.props.treeEntryStore.setNodeLeaf
    console.log('selected', selectedKeys);
  }

  async componentDidMount() {
    await this.props.treeEntryStore.getTree(this.props.url, this.props.expendId, this.props.filterOption)
  ...
  async xmlParseToModel( uploadFileName, pk_category){
    var resultData = await this.props.treeEntryStore.xmlParseToModel(uploadFileName, pk_category);
    var expectId = resultData.expectId;
    // 获取导入后的设计模型主键,刷新树并展开新导入的节点
    this.props.treeEntryStore.resetTempNode()
    // 刷新树对象
    this.props.treeEntryStore.getTree(this.props.url, expectId)
  ...
  async leafClick(node, paramFlag) {
    ...
    if(this.props.isRefTree) {
      ...
      this.props.treeEntryStore.setNextNodeInfo(node, '')
      this.props.treeEntryStore.setSelectedKeys([node.id]);
      return
    }
    if(this.props.hasClickCallBack) {
      let flag = this.props.leafClickCallBack && this.props.leafClickCallBack(node,this.props.treeEntryStore.tree.joinfield)
      if (flag){
        const { info } = this.props.treeEntryStore.tree
        this.setState({
    ...
    if (!node.isparent && !node.isRoot) { // 点击叶子节点
      if(this.props.isRefTree) {
        ...
        this.props.treeEntryStore.setLeafTapMsg(node, 'leafNode');
        this.props.treeEntryStore.setSelectedKeys([node.id]);
      ...
      this.props.treeEntryStore.setBranchInfo(node)
      window.mdmNowUrl = window.location.href;
  ...
  async onIconClick(e, item, type) {
    ...
    // 清空editInfo 内容
    this.props.treeEntryStore.setEditInfo({})
    this.props.treeRefStore.resetRefMsg({});
    const {treeEntryStore } = this.props;
    // 草稿、停用
    if (type === 'rootFolder' || type === 'branchFolder') { // rootFolder branchFolder branchAdd
      this.props.treeEntryStore.setModalMsg(item, type)
      this.props.treeEntryStore.setTreeModal(true)
    } else if (type === 'branchEdit' || type === 'leafEdit'){
      setTimeout(() => {
        this.props.treeEntryStore.setModalMsg(item, type)
      }, 100)
      await this.props.treeEntryStore.getDesignInfo(item.id);
      this.props.treeEntryStore.editNodeBranch(item, type)
      this.props.treeEntryStore.setTreeModal(true)
    } else if(type === 'branchAdd'){
      this.props.treeEntryStore.resetTempNode()
      this.props.treeEntryStore.setModalMsg(item, type)
      this.props.treeEntryStore.setTreeModal(true)
      this.props.treeEntryStore.getTree(this.props.url, this.props.expendId)
    } else if(type === 'importDesignModel'){
      this.setState({
        selecteCategory: item.id
      })
      this.props.treeEntryStore.setModalMsg(item, 'importDesignModel')
      ...
      this.props.history.push('/')
      // defaultExpandedKeys
      // this.props.treeEntryStore.setExpandedKeys()
      this.props.treeEntryStore.setNodeLeaf(item, type)
      this.props.treeEntryStore.resetTableStatus(true,false)
    }
  }
  onMouseLeave = () => {
    this.setState({
  ...
  onExpand(expandedKeys) {
    this.props.treeEntryStore.setExpandedKeys(expandedKeys);
  }
   ...
   */
  async delSubmit() {
    const { state } = this.props.treeEntryStore.modal.nodeInfo;
    const { nodeType } = this.props.treeEntryStore.modal;
    // 叶子节点才需要弹出
    if( state === 1 && nodeType === 'leafDel'){
        alert(<FormattedMessage id="js.com.tre1.0014" defaultMessage="该模型已启用，不能进行删除操作" />);
        return;
    }
    const { id } = this.props.treeEntryStore.modal.nodeInfo;
    const { url,expendId } = this.props;
    switch(nodeType){
      case "leafDel": //删除主数据
        await this.props.treeEntryStore.getEntitys(id);
        if(this.props.treeEntryStore.pk_mdentity){
          await this.props.treeEntryStore.delEntitys(this.props.treeEntryStore.pk_mdentity)
        }
        await this.props.treeEntryStore.delLeaf(id)

        break;
      case "branchDel":
        await this.props.treeEntryStore.delBranch(id)
        break;
      default:
        break;
    }
    this.props.treeEntryStore.getTree(url, expendId)
    this.closeDelModal()
    window.mdmNowUrl = window.location.href;
    this.props.history.push('/')
  }
  ...
  onDrop(info) {
    ...
    this.props.treeEntryStore.resetTreeInfo(showTree[0].children)
  }
  ...
  getInitTree() {
    ...
    const { tree, tree: {info} } = this.props.treeEntryStore  // 获取store下对象写法
    let tempInfo = info
  ...
  onDoubleClick (node) {
    ...
    const { expandedKeys, selectedKeys } = this.props.treeEntryStore.tree;
    ...
    if (!autoSelect) {
      ...
      !id ? id = this.props.treeEntryStore.branchInfo.id : null
    }else {
      ...
      <div>
        ...
        >
          ...
          <Modal.Footer className="text-center">
            ...
            <Button shape="border" onClick={this.delSubmit}>
