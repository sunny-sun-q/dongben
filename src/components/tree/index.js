...
class Siderbartree extends Component {
      ...
      onChange(info) {// 当文件正在上传，上传成功和上传失败触发的回调函数。 当上传状态发生变化，返回下列参数。
        ...
        if (info.file.status === 'done') {
          ...
          if('success' == info.file.response.flag){
            ...
            if(info.file.response.fileName === null || info.file.response.fileName === ""){
              Warning("文件上传成功,但没有返回上传后的文件名称,请确认集群文件上传系统是否配置正确")
            }else{
  ...
  onTreeSelect = (selectedKeys, info) => {
    // this.props.treeStore.setNodeLeaf
    console.log('selected', selectedKeys);
  }

  async componentDidMount() {
    await this.props.treeStore.getTree(this.props.url, this.props.expendId, this.props.filterOption)
  ...
  async xmlParseToModel( uploadFileName, pk_category){
    var resultData = await this.props.treeStore.xmlParseToModel(uploadFileName, pk_category);
    var expectId = resultData.expectId;
    // 获取导入后的设计模型主键,刷新树并展开新导入的节点
    this.props.treeStore.resetTempNode()
    // 刷新树对象
    this.props.treeStore.getTree(this.props.url, expectId)
  ...
  async leafClick(node, paramFlag) {
    ...
    if(this.props.isRefTree) {
      ...
      this.props.treeStore.setNextNodeInfo(node, '')
      this.props.treeStore.setSelectedKeys([node.id]);
      return
    }
    if(this.props.hasClickCallBack) {
      let flag = this.props.leafClickCallBack && this.props.leafClickCallBack(node,this.props.treeStore.tree.joinfield)
      if (flag){
        const { info } = this.props.treeStore.tree
        this.setState({
    ...
    if (!node.isparent && !node.isRoot) {
      ...
      if(this.props.isRefTree) {
        ...
        this.props.treeStore.setLeafTapMsg(node, 'leafNode');
        this.props.treeStore.setSelectedKeys([node.id]);
        window.mdmNowUrl = window.location.href;
        this.props.history.push(`/leaf/${node.id}/${node.code}`)
      }
    }else if( id && (node.isparent || node.isRoot) ){ //前一节点是否为枝干 && 主数据分类
      this.props.treeStore.setBranchInfo(node)
      window.mdmNowUrl = window.location.href;
  ...
  nodeContent(node,noChild) {
    ...
    switch(true){
      case node.isRoot && this.props.specialFlag :
      {
        ...
        downloadIcon = (
          <Tooltip inverse placement="bottom" overlay={<FormattedMessage id="js.rou.ctt.00381" defaultMessage="下载全部" />}>
            <Icon type="uf-download"
              onClick={(e) =>this.props.option.download(e,node)}
            />
          </Tooltip>
        );
        break;
      ...
      case (noChild && !node.isparent && (node.state === 0 || node.state === 2) && (node.publish === 0 || !node.publish)): //叶子节点，即数据模型
        ...
        checkIcon =  (
          <Tooltip inverse placement="bottom" overlay={checkText}>
            <Icon type="uf-table-s-o"
              onClick={(e) =>this.onIconClick(e,node, 'check')}
            />
          </Tooltip>
        );
        if(contextId !== 'mdm'){
    ...
    return (
      <div className="tree-branch" onClick={() => this.leafClick(node)} onDoubleClick={() => this.onDoubleClick(node)}>
        ...
        </div>
        {
          ...
          this.state.isHover === node.id && !this.props.ifNoHover && (this.props.showIcon!==false) ? 
          checkIcon || editIcon || addIcon || downloadIcon || ImportIcon || folderIcon || folderIcon || delIcon ? (<div className={`icon-group`} >
            {checkIcon}
            {editIcon}
            {addIcon}
            {downloadIcon}
  ...
  async onIconClick(e, item, type) {
    ...
    // 清空editInfo 内容
    this.props.treeStore.setEditInfo({})
    ...
    if (type === 'rootFolder' || type === 'branchFolder') { // rootFolder branchFolder branchAdd
      this.props.treeStore.setModalMsg(item, type)
      this.props.treeStore.setTreeModal(true)
    } else if (type === 'branchEdit' || type === 'leafEdit'){
      setTimeout(() => {
        this.props.treeStore.setModalMsg(item, type)
      }, 100)
      await this.props.treeStore.getDesignInfo(item.id);
      this.props.treeStore.editNodeBranch(item, type)
      this.props.treeStore.setTreeModal(true)
      ...
      this.props.treeStore.setModalMsg(item, 'importDesignModel')
      ...
      this.props.checkFun(item)
    } else {
      window.mdmNowUrl = window.location.href;
      this.props.history.push('/')
      // defaultExpandedKeys
      // this.props.treeStore.setExpandedKeys()
      this.props.treeStore.setNodeLeaf(item, type)
      this.props.treeStore.resetTableStatus(true,false)
    }
  }
  onMouseLeave = () => {
    this.setState({
   ...
   */
  async delSubmit() {
    const { state } = this.props.treeStore.modal.nodeInfo;
    ...
    switch(nodeType){
      case "leafDel": //删除主数据
        await this.props.treeStore.getEntitys(id);
  ...
  onDrop(info) {
    ...
    this.props.treeStore.resetTreeInfo(showTree[0].children)
  }
  ...
  getInitTree() {
    ...
    const { tree, tree: {info} } = this.props.treeStore  // 获取store下对象写法
    let tempInfo = info
  ...
  onDoubleClick (node) {
    ...
    const { expandedKeys, selectedKeys } = this.props.treeStore.tree;
    ...
    if (!autoSelect) {
      ...
      !id ? id = this.props.treeStore.branchInfo.id : null
    }else {
      ...
      <div>
        ...
        >
          ...
          <Modal.Footer className="text-center">
            <Button bordered style={{ marginRight: 20 }} onClick={this.closeDelModal}>
                <FormattedMessage id="js.com.tre1.0017" defaultMessage="取消" />
            </Button>
            <Button  onClick={this.delSubmit}>
