...
import { treeStore, dataMaintainStore, comboxStore, seniorSearchStore, dataEditStore, dataHistoryStore, dataProcessStore } from './stores/store'
import { cardStore } from '../../data-management/management/stores/store'
const render = (Component) => {
  ReactDOM.render(
    <AppContainer >
      <IntlProvider locale={locale.replace(/_.+/ig,'')} messages={chooseLocale(locale)}>
        <Provider treeStore={treeStore} 
          dataMaintainStore={dataMaintainStore} 
          comboxStore={comboxStore} 
          seniorSearchStore={seniorSearchStore} 
          dataEditStore={dataEditStore}
          dataHistoryStore = { dataHistoryStore }
          dataProcessStore = { dataProcessStore }
          cardStore = { cardStore }
          >
          <Component />
        </Provider>
