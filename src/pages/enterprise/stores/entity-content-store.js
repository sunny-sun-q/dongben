...
export default class EntityContentStore {
  ...
  // 获取企业客商详情的请求
  @action getTableDataRequest(pk_sys, gdCode, categoryCode, condition, pageIndex, pageSize) {
     
    return new Promise((resolve, reject) => {
      request('/distributeThirdPartService/getMasterDatas', {
        method: "GET",
        param: { pk_sys, gdCode, categoryCode, condition, pageIndex, pageSize }
      }).then((resp) => {
         
        this.tableDatas = {
          columns: resp.data.header,
          datas: resp.data.data,//mapTableData(resp.data.header, resp.data.data)
