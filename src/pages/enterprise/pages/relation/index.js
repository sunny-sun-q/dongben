  ...
  computedSymbol(arr, position, isDown) {
    return (
      <ul className="invest_tree_more_group">
        {
          arr.map((item, index) => <li className={!item.hasChildren?'has_no_more':null} key={`symbol-up-${item.id}`} onClick={ item.hasChildren?() => this.showMoreUp(item.holderName, position, index, arr.length, isDown, item.arrIndex):null}></li>)
        }
      </ul>
    )
  }
