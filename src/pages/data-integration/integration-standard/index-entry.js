...
import { treeStore, comboxStore, apiStore,menuStore,sysregisterStroe,reactJsonViewStore} from './stores/store'
const render = (Component) => {
  ReactDOM.render(
    <AppContainer >
      <IntlProvider locale={locale.replace(/_.+/ig,'')} messages={chooseLocale(locale)}>
        <Provider treeStore={treeStore} comboxStore={comboxStore} apiStore={apiStore}
                  menuStore={menuStore} sysregisterStroe={sysregisterStroe} 
                  ReactJsonViewStore = {reactJsonViewStore}>
          <Component />
        </Provider>
      </IntlProvider>
    </AppContainer>,
