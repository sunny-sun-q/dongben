...
import { interfaceConfigurationStore, treeStore, entityContentStore, comboxStore } from './stores/store'

const render = (Component) => {
  ReactDOM.render(
    <AppContainer >
      <IntlProvider locale={locale.replace(/_.+/ig,'')} messages={chooseLocale(locale)}>
        <Provider interfaceConfigurationStore={interfaceConfigurationStore} treeStore={treeStore} entityContentStore={entityContentStore} comboxStore={comboxStore}>
          <Component />
        </Provider>
      </IntlProvider>
    </AppContainer>,
